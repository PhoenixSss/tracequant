...
run `push-readiness --verify` against the same artifact and retain its exact
evidence result. Continue only after it passes. Re-read branch and remote-head
identity, then pass that exact `PUSH_VERIFY_RESULT` to `pr-readiness`; the gate
rejects a missing, stale, failed, or mismatched verify artifact.
