...
- Cross-workspace/provider Remediation preserves the existing Codex/local-record path and adds an explicit semantic-only `--findings-file` fallback when the originating ignored Review audit record is unavailable. Mechanical identity remains live-resolved; Remediation Complete uses the operation-local prepared start head rather than requiring the old Review record.
